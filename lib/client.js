  ...
  factory: (require) => {
    ...
    function makeSpeakToggle(opts) {
      ...
      return function SpeakToggle(props) {
        ...
        // Audio comes ONLY from the assistant's actual chat reply, never
        // from the speak/cheer tool's own text directly: a cheer's short
        // intro and the fuller reply that follows it in the SAME turn were
        // two independent triggers that could both generate and play audio
        // ("so many sentences" for one turn). A cheer's text still shows up
        // as its own tool-call line in the chat transcript (the host
        // renders that generically) — it just never reaches TTS on its own.
        //
