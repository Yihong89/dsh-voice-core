...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var store = { cheer: null, sessionId: undefined, voiceOpen: false, pendingFull: null, listeners: [] }
    var pluginCtx = null
    function emit() {
    ...
    // A user-requested "hear full reply" click is worth waiting longer for
    // — they explicitly opted in — but still needs a ceiling.
    ...
    function setActiveSession(sessionId) {
      ...
      emit() // let store subscribers (CheerChip, HearFullChip, ...) drop stale-session state
    }
    ...
    function makeSpeakBrowser(opts) {
      return function speakBrowser(text, sessionId, speakOpts) {
        if (typeof window === 'undefined' || typeof window.fetch !== 'function') return
        var full = speakOpts !== undefined && speakOpts !== null && speakOpts.full === true
        var prepared = speakable(text)
        ...
        if (full) {
          ...
          if (store.pendingFull !== null && store.pendingFull.sessionId === sessionId && store.pendingFull.presetName === opts.presetName) {
            store.pendingFull = null
            emit()
          }
        } else if (clean.length < prepared.length) {
          // Cut short for TTS — offer the user a way to hear the rest
          // instead of silently dropping it.
          store.pendingFull = { text: prepared, sessionId: sessionId, presetName: opts.presetName }
          emit()
        }
        ...
        // independent speakBrowser calls (each with its own dedup
        // tracking, unaware of the other) that can both finish generating
        // and play back-to-back. Only ever the latest request should be
        // audible — one voice, one line, always the newest.
        ...
        // queued behind it. A user-requested full reply gets more slack
        // since they explicitly opted in to the wait.
    ...
    function makeHearFullChip(opts) {
      return function HearFullChip() {
        var state = React.useState(store.pendingFull)
        React.useEffect(function () {
          return subscribe(function () {
            state[1](store.pendingFull)
          })
        }, [])
        var raw = state[0]
        var pending = raw && raw.presetName === opts.presetName && raw.sessionId === activeSessionId ? raw : null
        var visibleState = React.useState(false)
        React.useEffect(function () {
          if (pending === null) {
            visibleState[1](false)
          ...
          h('div', {
            ...
            h('button', {
              onClick: function () {
                ...
                store.pendingFull = null
                emit()
                speakBrowser(text, sessionId, { full: true })
              },
