...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * check) rather than props, since this component is registered under
     * shell.overlay — unlike conversation.input.right, it is not confirmed
     * to receive session props of its own.
     */
    function makeBackgroundLayer(opts) {
      return function BackgroundLayer() {
        ...
        if (!opts.backgroundUrl || state[0] !== opts.presetName) return null
        return h('div', {
          className: 'dsh-voice-background',
          style: {
