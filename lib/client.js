 ...
 *  - `shell.overlay` voice style picker panel + hear-full-reply chip.
 *
 ...
 * speak request for a turn is ever heard, hear-full UI state scoped to its
 * own preset) live in ONE place: `createVoiceController()` below. Every
 * component is a thin view over it — it reads snapshots via
 * voice.getPendingFull/getQueueStatus and calls
 * voice.request/playClip/setActiveSession/watchQueue. No component touches
 * a shared queue/session/abort variable directly. This module is loaded
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // other pending speak request behind it. Cap what actually gets sent.
    ...
    //
    // Every mounted preset's UI (SpeakToggle, HearFullChip) shares this one
    // instance: only one voice can be speaking at a time, and only one
    // companion's plugin is ever active for the session in view, so "what's
    // audible right now" has to be tracked in exactly one place instead of
    // by convention across N call sites. This controller is where that
    ...
    function createVoiceController() {
      ...
      // Only the latest speak request matters — an older one still in
      ...
      function stopAndClear() {
        ...
        // fetch keeps running server-side purely to be discarded by the
        // sessionId check once it resolves — wasted TTS-queue work the
      ...
      function setActiveSession(sessionId, presetName) {
        ...
        if (changed) emit() // let subscribers (HearFullChip, BackgroundLayer, ...) refresh
      }
      ...
      function playClip(url, sessionId) {
        ...
        if (audioPlaying || pendingController !== null || audioQueue.length > 0) return
       ...
       * assistant reply, or a user-requested full replay).
       * @param {object} req
      ...
      function request(req) {
        ...
        // (stopAndClear aborts pendingController too.)
        stopAndClear()
