...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * the one actually hiding anything set on document.body.
     */
    ...
    function makeBackgroundLayer(opts) {
      return function BackgroundLayer() {
        ...
        React.useEffect(function () {
          ...
          var opaque = appRoot ? findOpaqueAncestor(appRoot, 8) : null
          var previousOpaqueBg = opaque ? opaque.style.backgroundColor : null
          if (opaque) opaque.style.backgroundColor = 'transparent'
          return function () {
            for (var j = 0; j < BODY_BG_PROPS.length; j++) body.style[BODY_BG_PROPS[j]] = previous[BODY_BG_PROPS[j]]
            if (opaque) opaque.style.backgroundColor = previousOpaqueBg
          }
